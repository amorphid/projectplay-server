  ...
  def index
    ...
    respond_to do |format|
      ...
      format.json {
        ...
        # render :json => @playgrounds, :callback => params[:callback]
        if params[:callback]
          render :json => @playgrounds, callback: params[:callback], content_type: "application/javascript"
        else
  ...
  def nearaddy
    ...
    respond_to do |format|
      ...
      format.json {
        ...
        # render :json => @playgrounds, :callback => params[:callback]
        if params[:callback]
          render :json => { :origin => origin, :playgrounds => @playgrounds}, callback: params[:callback], content_type: "application/javascript"
        else
          render :json => { :origin => origin, :playgrounds => @playgrounds }
        end
        #render :json => { :origin => origin, :playgrounds => @playgrounds }
        }
...
def get_geo_from_google(addr1)
    address = ""
    address = addr1
    unless (address.downcase.match("alexandria") or address.downcase.match("arlington"))
      address = address + "+Alexandria+VA"
    end
  ...
  def getPlaygrounds
    ...
    # Default to Alexandria if Alexandria or Arlington is not specified
    addy = params[:address]
    unless (addy.downcase.match("alexandria") or addy.downcase.match("arlington"))
      addy = addy + ", Alexandria VA"
    end
    usergeo = get_geo_from_google(addy)
    ...
    respond_to do |format|
      ...
      format.json {
        ...
        # render :json => @playgrounds, :callback => params[:callback]
        if params[:callback]
          render :json => @playgroundset, callback: params[:callback], content_type: "application/javascript"
        else
          render :json => @playgroundset, :callback => params[:callback]
        end
        #render :json => @playgroundset, :callback => params[:callback]
        }
    end
  ...
  def show
    ...
    respond_to do |format|
      ...
      format.json {
        ...
        # render :json => @playgrounds, :callback => params[:callback]
        if params[:callback]
          render :json => @playground, callback: params[:callback], content_type: "application/javascript"
        else
          render :json => @playground
        end
        #render :json => @playground, :callback => params[:callback]
        }
    end
  end
  ...
  def create
    @playground = Playground.new(app_params)
  ...
  private 
	def app_params
		params.require(:playground).permit(:access, :agelevel, :playclass, :compsum, :conditions,
 ...
 :socialdom, :specificcomments, :subarea, :totplay, :weather, :lat, :long,
  :google_placesid, :imageurl, :address)
  end
